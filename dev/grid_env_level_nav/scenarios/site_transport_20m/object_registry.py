#!/usr/bin/env python3
"""ObjectRegistry: AI Sight → semantic slot tracking (never rasterized to costmap)."""
...
class ObjectRegistry:
    """Semantic object memory from AI Sight — queried for goals, never painted to L2."""

    entries: Dict[str, RegistryEntry] = field(default_factory=dict)

    def get(self, slot_id: str) -> Optional[RegistryEntry]:
        return self.entries.get(slot_id)

    def goal_xy(self, slot_id: str) -> Optional[WorldXY]:
        entry = self.entries.get(slot_id)
        return entry.last_world_xy if entry is not None else None

    def goal_local(self, slot_id: str) -> Optional[Tuple[float, float]]:
        xy = self.goal_xy(slot_id)
        return lc.world_xy_to_local(*xy) if xy is not None else None
    ...
    def upsert(
        ...
        *,
        ...
        entry = RegistryEntry(
            ...
            last_world_xy=world_xy,
            last_seen_t=seen_t if seen_t is not None else time.time(),
            is_dynamic=is_dynamic,
        )
        self.entries[slot_id] = entry
        return entry

    def evict_dynamic(self, slot_id: str) -> bool:
        entry = self.entries.get(slot_id)
    ...
    def clear_dynamic(self) -> None:
        for slot_id in list(self.entries.keys()):
            if self.entries[slot_id].is_dynamic:
                del self.entries[slot_id]
...
def update_object_registry_from_sight(
    ucv: UnrealCV,
    ...
    *,
    ...
    """AI Sight → ObjectRegistry update (log-only, no costmap writes)."""
    ...
    for slot_id, entry in list(registry.entries.items()):
        if not entry.is_dynamic or slot_id in visible_now:
            continue
        if _position_in_sensor_view(robot_xy, robot_yaw, entry.last_world_xy, config=cfg):
            if registry.evict_dynamic(slot_id):
                entries_evicted += 1
    ...
    for slot_id, target in visible_now.items():
        ...
        if live_xy is None:
            existing = registry.get(slot_id)
            if existing is not None:
                live_xy = existing.last_world_xy
            else:
                continue
        registry.upsert(
            slot_id=slot_id,
