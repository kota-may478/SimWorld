...
SEARCH_SPIN_PERIOD_S = 2.0  # One full rotation in ~2 seconds
...
class PersonDetector:
    ...
    def detect(self, frame_bgr: np.ndarray) -> Optional[Dict[str, object]]:
        ...
        if self.far_upsample > 1.01:
            up = cv2.resize(processed, dsize=None, fx=self.far_upsample, fy=self.far_upsample, interpolation=cv2.INTER_CUBIC)
        ...
        # Temporal hold stabilizes sparse misses at far range.
        if self.last_det is not None and (time.time() - self.last_det_ts) <= 0.25:
            hold = dict(self.last_det)
            hold["confidence"] = float(hold.get("confidence", 0.0)) * 0.9
    ...
    def _iter_tiles(self, width: int, height: int) -> List[Tuple[int, int, int, int]]:
        ...
        # Add overlap-based starts for finer far-range scanning.
    ...
    def _detect_hog_tiled(self, frame_bgr: np.ndarray) -> List[Dict[str, object]]:
        ...
        for x0, y0, x1, y1 in self._iter_tiles(W, H):
            ...
            if self.far_upsample > 1.01:
                tile_up = cv2.resize(tile, dsize=None, fx=self.far_upsample, fy=self.far_upsample, interpolation=cv2.INTER_CUBIC)
                candidates.extend(
...
def set_latest_sensor_state(state: Dict[str, object]) -> None:
    ...
    with sensor_lock:
        ...
        global latest_sensor_state_ts
        latest_sensor_state_ts = now_ts

        if np.isfinite(range_cm):
            sensor_range_history.append((now_ts, range_cm))
...
def get_latest_camera_frame_with_ts() -> Tuple[Optional[np.ndarray], float]:
    with sensor_lock:
        if latest_camera_frame is None:
            return None, latest_camera_frame_ts
        return latest_camera_frame.copy(), latest_camera_frame_ts
...
def record_range_fallback_sample() -> None:
    try:
        ...
        with sensor_lock:
            sensor_range_history.append((time.time(), dist_cm))
    except Exception:
        pass


def force_refresh_monitor_frame(reason: str = "") -> None:
    try:
        update_sensor_camera_pose()
        rgb = communicator.get_camera_observation(SENSOR_CAMERA_ID, "lit", mode="direct")
        frame = draw_sensor_overlay(
            rgb,
            None,
            float("nan"),
            "monitor_direct_preview",
            searching=True,
        )
    except Exception as exc:
        frame = draw_sensor_overlay(
            ...
            None,
            float("nan"),
            "monitor_preview_error",
            searching=True,
        )
        cv2.putText(
            frame,
            f"preview error: {str(exc)[:54]}",
            (10, 104),
...
def monitor_loop() -> None:
    ...
    stale_threshold_s = max(0.8, 2.0 * dt)

    try:
        while not monitor_stop_event.is_set():
            frame, frame_ts = get_latest_camera_frame_with_ts()
            now_ts = time.time()

            if (frame is None or (now_ts - frame_ts) > stale_threshold_s) and not simulation_done_event.is_set():
                force_refresh_monitor_frame("monitor fallback: AGV frame update stale")
                record_range_fallback_sample()
                frame, frame_ts = get_latest_camera_frame_with_ts()
            ...
            # Keep windows responsive and allow manual close.
